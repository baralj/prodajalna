...
var pesmiIzRacuna = function(racunId, callback) {
    ...
    function(napaka, vrstice) {
      if (napaka)
        console.log("Napaka pri dostopu do pesmi iz računa.");
      else
        callback(vrstice[0]);
    })
}
...
var strankaIzRacuna = function(racunId, callback) {
    ...
    function(napaka, vrstice) {
      if (napaka)
        console.log("Napaka pri dostopu do stranke iz računa.");
      else
        callback(vrstice[0]);
    })
}
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
    ...
    form.parse(zahteva, function (napaka1, polja, datoteke) {
      pesmiIzRacuna(parseInt(polja.seznamRacunov,10), function(pesmi) {
          strankaIzRacuna(parseInt(polja.seznamRacunov,10), function(racunStranka) {
            odgovor.setHeader('content-type','text/xml');
            odgovor.render('eslog', {
                vizualiziraj: true,
                postavkeRacuna: pesmi,
                stranka: racunStranka
            })
         })
      })
    })
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.trenutnaStranka = undefined;
    odgovor.redirect('/prijava') 
})
